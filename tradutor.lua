...
-- Tradução intllib
treasures_loot_nodes.intllib = {}
treasures_loot_nodes.intllib.S, treasures_loot_nodes.intllib.NS = dofile(modpath.."/lib/intllib.lua")

-- Configura tradutor opicional
treasures_loot_nodes.S = treasures_loot_nodes.intllib.S
treasures_loot_nodes.NS = treasures_loot_nodes.intllib.NS
...
-- Gera arquivos de tradução treasures_loot_nodes.*.tr
do
	...
	for _,file in ipairs(list) do
		...
		if string.match(file, "~") == nil then
			...
			if string.match(file, ".pot") == nil and string.match(file, ".po") then
				...
				-- Novo arquivo
				local new_file = "### Arquivo gerado por treasures_loot_nodes apartir de "..file.."\n# textdomain: treasures_loot_nodes\n"
				for en,lang in pairs(en_to_lang) do
					new_file = new_file .. en .. "=" .. lang .. "\n"
				end
				-- Escrever arquivo
				local saida = io.open(modpath.."/locale/treasures_loot_nodes."..lang_code..".tr", "w")
...
if minetest.get_translator ~= nil then
	s = minetest.get_translator("treasures_loot_nodes")
else
	s = treasures_loot_nodes.intllib.S
end

treasures_loot_nodes.s = function(...)
	local args = { ... }
	if pt_to_en[args[1]] ~= nil then
		return s(pt_to_en[args[1]], unpack(args, 2))
	end
	minetest.log("error", "[treasures_loot_nodes] String "..dump(args[1]).." nao catalogada")
	return s(...)
end
	...
	if type(args[1]) == "table" then
		...
		for n,a in ipairs(args[1]) do
			if n ~= 1 then -- Não traduz o primeiro
				table.insert(r, treasures_loot_nodes.S(a))
			else
				table.insert(r, a)
			end
		end
		
		return treasures_loot_nodes.s(unpack(r))
		
	elseif type(args[1]) == "string" then
		-- Não traduz caso faltem argumentos (devido strings ilustrativas)
		return treasures_loot_nodes.s(...)
...
treasures_loot_nodes.Sfake = function(s) return s end
